...
/*   Updated: 2021/06/23 13:42:07 by ccartman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "printf.h"

static char	*ft_bigint_to_ascii(int sign, int exp, unsigned long mant);
...
char	*ft_intpart_to_ascii(int sign, int exp, unsigned long mant)
{
	...
	else if (exp < (int)(sizeof(long) * 8))
	{
		...
		return (ft_bigint_to_ascii(sign, exp - MANT_SIZE, mant)); 
}
