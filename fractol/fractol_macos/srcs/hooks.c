...
/*   Updated: 2021/10/24 18:17:14 by ccartman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	deal_release(int button, int x, int y, t_fractol *fr);
...
void	define_hooks(t_fractol *fr)
{
	...
	mlx_hook(fr->win_ptr, 5, 1L << 1, deal_release, fr);
	mlx_hook(fr->win_ptr, MOTION_NOTIFY, 1L << 8, \
...
static int	deal_motion_notify(int x, int y, t_fractol *fr)
{
	if (ft_strcmp(fr->name, JULIA) == 0 && fr->right_button)
	{
